...
void packet_generate(terminal_state * s, tw_bf * bf, terminal_message * msg, tw_lp * lp)
{
  ...
  int num_chunks = msg->packet_size / chunk_size;
...
//  Each chunk is 32B and the VC occupancy is in chunks to enable efficient flow control
   ...
   if(s->vc_occupancy[vc] >= (cn_vc_size * msg->num_chunks))
...
/* collective operation for the dragonfly network */
...
#if DEBUG
if( msg->packet_ID == TRACK && next_stop != msg->dest_terminal_id && msg->chunk_id == msg->num_chunks-1)
  {
 ...
 // If source router doesn't have global channel and buffer space is available, then assign to appropriate intra-group virtual channel 
  ...
  if(next_stop == msg->dest_terminal_id)
  {
    ...
    if(s->vc_occupancy[output_chan] >= cn_vc_size * msg->num_chunks)
  ...
  else
  {
   ...
   if( global )
   {
     if(s->vc_occupancy[output_chan] >= global_vc_size * msg->num_chunks )
       s->output_vc_state[output_chan] = VC_CREDIT;
   }
  else
    {
     /* buffer space is less for local channels */
     if( s->vc_occupancy[output_chan] >= local_vc_size * msg->num_chunks )
	s->output_vc_state[output_chan] = VC_CREDIT;
    }
  }
  tw_event_send(e);
			...
			tw_lp * lp )
{
    ...
    msg->num_chunks = msg->packet_size/chunk_size;
...
void terminal_rc_event_handler(terminal_state * s, tw_bf * bf, terminal_message * msg, tw_lp * lp)
{
   switch(msg->type)
   {
	   ...
	   case T_BUFFER:
		   ...
		   if(s->vc_occupancy[msg_indx] == cn_vc_size * msg->num_chunks)
  ...
  switch(msg->type)
    {
	    ...
	    case R_BUFFER:
			 ...
			 buf = cn_vc_size;
		      ...
		      if(s->vc_occupancy[msg_indx] >= buf * msg->num_chunks)
                          s->output_vc_state[msg_indx] = VC_CREDIT;
