...
mpirun -np 2 ${basePath}/install-test/bin/traceR --lp-io-dir=${testPath}/output/test-output-${confName} --sync=3 -- ../conf/${confName}.conf tracer_config | sed -n '/START PARALLEL OPTIMISTIC SIMULATION/,/END SIMULATION/p' | tail -n +2 | head -n -3 > std.out
rv=$?
cat std.out
